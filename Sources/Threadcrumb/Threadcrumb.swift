...
final public class Threadcrumb: @unchecked Sendable {
    ...
    public init(identifier: String) {
        ...
        let completeSemaphore = self._completedSemaphore
    ...
    public func log(_ value: String) {
        ...
        // Normalize the value to lowercase a-z, 0-9, and underscore because these map to the available function symbols.
    ...
    /// Allowed and disallowed character sets used for input sanitization.
    static private let _sAllowedCharacters: CharacterSet = CharacterSet(
        charactersIn: "0123456789abcdefghijklmnopqrstuvwxyz_"
    )
    static private let _sDisallowedCharacters: CharacterSet =
        _sAllowedCharacters.inverted

    /// Serializes calls to `log()` and protects handoff state.
    fileprivate let _lock: OSAllocatedUnfairLock = OSAllocatedUnfairLock()

    /// The sanitized characters to encode as a breadcrumb.
    ...
    /// The latest captured call stack from the background thread.
...
extension Array {
    /// Removes and returns the first element of the array, mirroring `Collection.popFirst()` behavior for arrays.
...
/// Starts the per-character function chain by dispatching on the first character.
/// Recurses until all characters are exhausted and then calls `THREAD_CRUMB_END`.
@available(iOS 16.0, macOS 13.0, tvOS 16.0, watchOS 9.0, visionOS 1.0, *)
...
/// Maps allowed characters to their unique function symbols so that each character appears distinctly in the call stack.
@available(iOS 16.0, macOS 13.0, tvOS 16.0, watchOS 9.0, visionOS 1.0, *)
private let _lookupTable:
    [String: @Sendable (Threadcrumb, inout [String]) -> Void] = [
        "0": THREAD_CRUMB__0,
        "1": THREAD_CRUMB__1,
        "2": THREAD_CRUMB__2,
        "3": THREAD_CRUMB__3,
...
extension Threadcrumb {
    ...
    internal func stringLoggingThread() -> String {
